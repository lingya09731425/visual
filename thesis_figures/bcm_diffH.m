...
for H_p = [1.5 : 0.5 : 4.5 Inf]
        ...
        if exist(subfolder_name, 'dir')
            ...
            % W_end = mean(W_evo(:,:,(num_of_plots - 1000) : num_of_plots), 3);
            W_end = W_evo(:,:,(num_of_plots - 1000) : num_of_plots);

            active = all(W_end > W_thres(2) / 20, 3);
            bar_width = sum(active, 2);
            avg_width = mean(bar_width(bar_width > 3));
            sd_width = std(bar_width(bar_width > 3));
            sparsity = sum(bar_width > 3) / N_out;
