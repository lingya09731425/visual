...
subfolder_name = sprintf(['%s/%s_recurrent_%s_bias%.2f_' ...
...
gaba_name = sprintf('%s/gaba.mat', subfolder_name);
eventlog = fopen(sprintf('%s/eventlog.txt', subfolder_name), 'w');

% run simulation
[record_times,record_W,record_output,record_theta,record_gaba,plot_times,plot_W] = ...
    independent_rates( ...
        type, ...
...
save(gaba_name, 'record_gaba', 'record_times');
