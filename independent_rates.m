function W = independent_rates(biased_W, bias, random_L_events, N_in, N_out, total_ms, dt_per_ms, ...
    ...
    % preallocated record storage for some stats
    % avg: mean of all weights
    % W_all: entire weights
    % out_all: accumulation of output activity between snapshot points
    num_of_records = round(total_ms / plot_W_freq) + 1;
    avg = zeros(1, num_of_records);
    avg(1) = mean(mean(W));
    W_all = zeros(N_out, N_in, num_of_records);
    W_all(:,:,1) = W;
    out_all = zeros(N_out, num_of_records);
